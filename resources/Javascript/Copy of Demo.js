function fetchDetails(service){	
		if(service=="ais"){
			var accountNumber  = document.getElementById("cIBAN3").value;	
			var searchInfo = document.getElementById("searchacc").value;		
			...
			if( startDt > now || endDt > now || startDt > endDt){
				...
				var urlLink="http://10.21.194.250:9082/AISRestTransferLayer/rest/users";
				//var urlLink="http://localhost:9081/AISRestTransferLayer/rest/users";
		...
		function iterateData(responsetxt,dataTypeReq,service){
			...
			if(service=="tpp"){
				...
				if(dataTypeReq=="xml"){
					...
					if(format=="BSI271" || format=="CAMT53")
					{
						customers.push(["Mutation Code","Book Date","Value Date","Currency Code","Amount","Balance After Transaction","Description","Name Counter Account","Counter Account Number","Transaction Date","Source Enquiry Number"]);
						...
						customers.push(["CounterParty Account","CounterParty Name","Currency Code","Amount","Error Code","Remittance Information"]);
						$.each(responsetxt, function(k,v) {  
							customers.push([v["counterAccountNumber"],v["nameCounterAccount"],v["currencyCode"],v["amount"],v["errorCode"],v["remittanceInfo"][0]]);											
						});		
					}
				}
			}
